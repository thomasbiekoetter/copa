program copa__test_rosenbrock_gaussianprior
  ...
  use copa__prior_functions, only : gaussian_prior
  ...
  call store_chains(  &
    chains,  &
    "plots/rosenbrock_gaussianprior/chains.npy",  &
    mode="machine")

  call store_log_probs(  &
    log_probs,  &
    "plots/rosenbrock_gaussianprior/log_probs.npy",  &
    mode='machine')
...
contains
  ...
  subroutine log_prior(theta, logp)
    ...
    real(wp) :: mu(size(theta))

    mu = 2.0e0_wp

    call gaussian_prior(theta, mu, logp)
...
end program copa__test_rosenbrock_gaussianprior
